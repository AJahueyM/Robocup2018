...
class Node {
public:
	Node(int x, int y) { this->x = x; this->y = y; g = 0; h = 0; f = 0; hasWallUp = false; hasWallDown = false; hasWallLeft = false; hasWallRigth = false; parent = nullptr; bumpType = None; wasVisited = false; isRamp = false; };
	...
	void setIsRamp(bool isRamp) { this->isRamp = isRamp; }
	bool getIsRamp() { return this->isRamp; }

	void clear() { this->setF(0); this->setG(0); this->setH(0); this->setParent(nullptr); }
...
private:
	int x, y, f, g, h;
	bool hasWallUp, hasWallDown, hasWallLeft, hasWallRigth, wasVisited, isRamp; 
	BumpLevel bumpType;
